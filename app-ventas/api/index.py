...
with app.app_context():
    ...
    if not Usuario.query.filter_by(username="admin").first():
        db.session.add(Usuario(username="admin", password=generate_password_hash("admin123"), role="ADMIN"))
        db.session.commit()
...
def dashboard():
    ...
    return render_template("dashboard.html", ventas=ventas, pagos=pagos,
...
def crear_usuario():
    ...
    if request.method == "POST":
        ...
        if Usuario.query.filter_by(username=username).first():
            ...
            db.session.add(Usuario(username=username, password=generate_password_hash(password), role=role))
            db.session.commit()
